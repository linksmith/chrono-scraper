...
class E2ETestRunner:
    ...
    async def test_1_user_signup_and_approval(self):
        ...
        self.user_data = {
            ...
            "username": f"researcher_{uuid.uuid4().hex[:8]}",
            "password": "TestSecurePassword123!",
            "full_name": fake.name(),
            "professional_info": {
                "title": "Senior OSINT Researcher",
                "organization": "Digital Forensics Institute",
                "research_focus": "Historical web content analysis and cyber threat intelligence research",
                "intended_use": "Academic research on information warfare and disinformation campaigns",
                "credentials": "PhD in Computer Science, 8 years experience in OSINT",
                "references": "Previously published research on web archiving methodologies"
            }
        }
        ...
        # Check that user is pending approval
        assert signup_data.get("is_approved") == False
        assert signup_data.get("approval_status") == "pending"
        
        # Simulate LLM approval process (in real scenario, this would be automatic)
        print("⏳ Waiting for LLM approval simulation...")
        
        # Check approval status endpoint
        approval_check = await self.client.get(f"/api/v1/users/{self.user_id}/approval-status")
        print(f"Approval status: {approval_check.json()}")
        
        # For testing, we'll manually approve the user
        # In production, this would be handled by the LLM evaluation task
        approval_data = {
            "approved": True,
            "feedback": "Excellent professional profile with clear research focus",
            "confidence_score": 0.92
        }
        
        # This endpoint would typically be internal/admin only
        admin_approval = await self.client.post(
            f"/api/v1/users/{self.user_id}/approve", 
            json=approval_data
        )
        
        if admin_approval.status_code == 200:
            print("✅ User approved successfully")
        else:
            print(f"⚠️ Manual approval failed, checking if auto-approval occurred")
            # Check if user was auto-approved (possible in test environment)
            user_check = await self.client.get(f"/api/v1/users/{self.user_id}")
            user_data = user_check.json()
            if user_data.get("is_approved"):
                print("✅ User was auto-approved")
            else:
                raise AssertionError("User approval failed")
    ...
    async def test_2_login_and_jwt_handling(self):
        ...
        assert profile_data["is_approved"] == True
